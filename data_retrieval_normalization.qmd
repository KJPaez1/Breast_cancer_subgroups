...
It is ommited for metabolic genes, but if i wouldn't, it would be based on the smallest group size for sample_type
...
ref: https://www.nature.com/articles/nbt.2530 ref: the main
...
tumor_samples_ID # 905
nontumor_samples_ID # 113


tumor_expr_matrix <- assay(vsd_metab_genes)[, tumor_samples_ID]
nontumor_expr_matrix = assay(vsd_metab_genes)[, nontumor_samples_ID]
...
sample_type = as.factor(c(rep("Tumor vs Normal", 226), 
                rep("Tumor vs Tumor", 905),
                rep("Normal vs Normal", 113)))

sample_type_0 = as.factor(c(rep("T vs N", 226),
                            rep("T vs T", 226), 
                            rep("T vs N combined", 905),
                            rep("N vs N", 113)))

lev = c(rep("N vs N", 113), rep("T vs N combined", 226), rep("T vs T", 905), rep("T vs N", 113))
...
#d_tumor_vs_non_tumor_0
n_t_vs_nont <- lengths(d_tumor_vs_non_tumor)
n_t_vs_nont_0 <- lengths(d_tumor_vs_non_tumor_0)

d = unlist(d_tumor_vs_non_tumor) # combine
d_0 = unlist(d_tumor_vs_non_tumor_0) # combine


g_mine <- rep(factor(levels(sample_type), 
                levels = levels(sample_type)), n_t_vs_nont) # combine
...
tumor_vs_normal_113_m = 
  cbind(t(tumor_vs_normal_113), group)
...
# Convert the numeric vector into a matrix (example assumes it's symmetric and square)
matrix_size <- sqrt(length(numeric_vector))
matrix_data <- matrix(numeric_vector, nrow = matrix_size, ncol = matrix_size)

# Convert the matrix to a dist object
dist_object <- as.dist(matrix_data)

# Include the dist object in a list
dist_list <- list(dist_object)
